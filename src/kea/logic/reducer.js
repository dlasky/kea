...
function warnIfUndefinedActionCreator(object, property) {
  if (process.env.NODE_ENV !== "production") {
    if (object.reducer.undefined !== undefined) {
      console.warn(
        `A reducer with the property "${property}" is waiting for an action where its key is not defined.`
      );
    }
  }
...
function warnIfMissingReducerObjectkeys(object, key) {
  if (process.env.NODE_ENV !== "production") {
    if (!object.value) {
      console.warn(`Reducer "${key}" is missing an initial state value`);
    }
    if (!object.reducer) {
      console.warn(`Reducer "${key}" is missing a reducer function`);
    }
  }
}
...
export function convertReducerArrays(reducers) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (Array.isArray(s)) {
      ...
      const options =
        typeof s[s.length - 2] === "object" ? s[s.length - 2] : undefined;
    ...
    } else if (Object.isObject(s)) {
      warnIfMissingReducerObjectkeys(s, keys[i]);
      reducers[keys[i]] = warnIfUndefinedActionCreator(s, keys[i]);
    }
  }
